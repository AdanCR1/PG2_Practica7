...
try:
    ...
    from pygame_ui_items.modal import Modal, create_confirm_modal
    from pygame_ui_items.ui_element import UIElement
    
    # --- Importar Estilo Sólido ---
...
# (Para v0.0.3, deberías mover esto a sus propios archivos y exportarlos)
...
pg.display.set_caption("Demo de GUI Completa - Estilo SÓLIDO")
clock = pg.time.Clock()
...
def open_quit_dialog():
    confirm_modal = create_confirm_modal(
        SCREEN_WIDTH, SCREEN_HEIGHT, "¿Seguro que quieres salir?",
        on_confirm=lambda: sys.exit(), on_cancel=close_top_window
    )
...
def create_main_menu() -> Window:
    ...
    menu.add_element(GuiPanel(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, bg_color=(30, 30, 50)))
    menu.add_element(GuiLabel(SCREEN_WIDTH // 2, 100, "MENÚ PRINCIPAL (SÓLIDO)", font_size=50, color=(255, 255, 255)))
    
    menu.add_element(button_green("Iniciar Juego", open_game_screen, 
                                  x=SCREEN_WIDTH // 2 - 100, y=250, width=200, height=50))
    menu.add_element(button_purple("Opciones", open_options_menu,
                                   x=SCREEN_WIDTH // 2 - 100, y=310, width=200, height=50))
    menu.add_element(button_red("Salir", open_quit_dialog,
...
def create_options_menu() -> Window:
    ...
    options.add_element(GuiPanel(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, bg_color=(50, 30, 30)))
    options.add_element(GuiLabel(SCREEN_WIDTH // 2, 100, "Opciones", font_size=50, color=(255, 255, 255)))
    
    options.add_element(button_gray("Volumen: [ON]", lambda: print("Click Volumen"),
                                   x=SCREEN_WIDTH // 2 - 150, y=250, width=300, height=50))
    options.add_element(button_gray("Dificultad: [Fácil]", lambda: print("Click Dificultad"),
                                   x=SCREEN_WIDTH // 2 - 150, y=310, width=300, height=50))
    options.add_element(button_red("Volver", close_top_window,
                                 x=SCREEN_WIDTH // 2 - 100, y=450, width=200, height=50))
    return options

def create_game_screen() -> Window:
    game = Window(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)
    game.add_element(GuiPanel(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, bg_color=(30, 50, 30)))
    game.add_element(GuiLabel(SCREEN_WIDTH // 2, 300, "¡El Juego está corriendo!", font_size=40, color=(255, 255, 255)))
    game.add_element(button_gray("Pausa [P]", open_pause_menu,
...
def create_pause_menu() -> Modal:
    ...
    pause_menu.add_element(button_green("Reanudar", close_top_window,
                               x=pause_menu.rect.centerx - 100, y=pause_menu.rect.top + 100, width=200, height=50))
    pause_menu.add_element(button_red("Menú Principal", return_to_main_menu,
